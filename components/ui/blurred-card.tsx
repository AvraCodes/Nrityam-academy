...
export const ShaderCanvas = ({ className }: { className?: string }) => {
  ...
  const [backgroundColor, setBackgroundColor] = useState([0.01, 0.01, 0.01]); // dark by default

  useEffect(() => {
    const root = document.documentElement;
    const updateColor = () => {
      const isDark = root.classList.contains('dark') || getComputedStyle(document.body).backgroundColor === 'rgb(3, 3, 3)';
      // Using a very dark color for dark mode and light for light mode
      setBackgroundColor(isDark ? [0.01, 0.01, 0.01] : [1.0, 1.0, 1.0]);
    };
    updateColor();
...
export const BlurredCard = ({ children, className }: { children: React.ReactNode, className?: string }) => {
  return (
    <div className={cn(
      "relative rounded-2xl shadow-xl flex flex-col transition-all duration-300 overflow-hidden",
      "border border-white/10 group-hover:border-primary/40",
      className
    )}>
      <div className="relative z-10 w-full h-full-[40px] bg-black/20">
        {children}
      </div>
    </div>
  );
